...
Profer 是一个基于proma开发的本地优先的 AI 桌面应用，把多模型 Chat、通用 Agent、工作区、Skills、MCP、远程机器人和记忆能力放在同一个开源客户端里。
...
1. 打开 Profer，先完成环境检查。Agent 模式依赖本机基础环境，尤其是 Git、Node.js / Bun 以及可用的 Shell。
...
### 流式语音输入(支持全局输入)
Profer 支持豆包的流式语音输入功能，并且支持在 Profer 内使用和 Profer 外部使用：
- Profer 内部使用：Ctrl + ` 触发识别，再次按下结束自动输入到 Profer 内对应的输入框
- Profer 外部使用：Ctrl + ` 触发识别，再次按下结束自动输入到当前的光标所在处，如无光标则默认写入到剪贴板
- 
